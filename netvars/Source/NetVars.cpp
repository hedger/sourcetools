...
namespace NetVars
{
	...
	const char* FixBaseEnt(const char* inName)
	{
		if (!inName)
			return NULL;
		if (!strcmp(inName, "CBaseEntity"))
			return "Net_BaseEntity";
		else
			return inName;
	}
	...
	{
		CNTClass *ret =  Analyze( NULL, pClass->m_pRecvTable, 0, true, pClass->GetName());
		if (ret)
			ret->m_pszName = pClass->GetName();
		return ret;
	}

	CNTClass* CNTClass::Analyze( CNTClass *pParent, RecvTable *pTable, int offset, bool addtolist, const char* name )
	{
		m_pTable = pTable;
		m_nBase = offset;
		m_pParent = pParent;

		m_pszName = FixBaseEnt(name);
		
		// Check if this table has already been analyzed
		CNTClass *current = Find( m_pTable->GetName() );
		if ( current ) return current;
		// Eventually add to global lits
		if ( addtolist )
		{
	...
	void CNTClass::ExportCode( std::ostream &out, int depth )
	{
		...
		std::string pre, name = GetStructName( !depth );
		for ( int i = 0; i<depth; i++ )
			pre.append("\t");
		// Variables we didn't understand (excludes baseclass and some stuff that is auto discarded)
		int failcount = 0;
		out << pre << "struct " << name;
		if (!name.compare("Net_BaseEntity"))
			out << " : public CBaseEntity";
		if ( m_pParent && !depth ) 
		...
		if (!m_Vars.empty())
		{
			out << pre << "\t#undef PROPPATH\n";
			out << pre << "\t#define PROPPATH \"" << BuildPath() << "\"\n";
	...
	std::string CNTClass::GetStructName( bool forcegendt )
	{
		...
		if (m_pszName)
		{
			return FixBaseEnt(m_pszName);
		}
		else 
			return m_pTable->GetName();
	}

	std::string CNTClass::BuildPath() const
	{
		std::string out;
		for (const CNTClass* parent = this; parent != nullptr; parent = parent->m_pParent)
		{
			std::string clsName = (parent->m_pszName) ? parent->m_pszName : parent->m_pTable->GetName();
			out.insert(out.begin(), clsName.begin(), clsName.end());
			if (parent->m_pszName)
				break;
	...
	std::string CNTVar::BuildPath() const
	{
		std::string out;
		for (CNTClass* parent = m_pParent; parent != nullptr; parent = parent->m_pParent)
		{
			std::string clsName = (parent->m_pszName) ? parent->m_pszName : parent->m_pTable->GetName();
			out.insert(out.begin(), clsName.begin(), clsName.end());
			if (parent->m_pszName)
	...
	bool CNTVar::ExportCode( std::ostream &out, int depth )
	{
		...
		if ( m_nArraysize>0 )
		{
			//inline float(&m_flEncodedController())[4] { static DynNetvar var("m_flEncodedController", "CBaseAnimating"); return MakeArrayReference< float, 4 >( this, var() ); } // Array size = 4
			out << "DECL_NETPROP_ARR( " << var << ", " << type << ", " << m_nArraysize << " )"; //", \"" << BuildPath() <<"\" )";

			//out << "inline " << type << "(& " << var << "())[" << m_nArraysize << "] { "; 
			//out << "static DynNetvar var(\"" << var << "\", \"" + BuildPath() + "\"); ";
			//out << "return MakeArrayReference< " << type << ", " << m_nArraysize << " >( this, var() ); }";
		}
		else
		{
			// static DynNetvar var("m_iTeamNum", "CBaseEntity"); return MakePointer< int >( this, var() ); 
			out << "DECL_NETPROP( " << var << ", " << type << " )"; //", \"" << BuildPath() <<"\" )";

			//out << "inline " << type << "& " << var << "() { "; 
			//out << "static DynNetvar var(\"" << var << "\", \"" + BuildPath() + "\"); ";
			//out << "return MakeReference< " << type << " >( this, var() ); }";
		};
	...
	void GenCode( const char *file )
	{
		...
		out << "#include \"DynNetvar.h\"\n\n";
		out << "namespace NetVar\n{\n";

		out << "template<typename T> inline T& MakeReference( PVOID thisptr, int offset ) { return *reinterpret_cast<T*>( reinterpret_cast<UINT_PTR>( thisptr ) + offset ); }\n";
		out << "template<typename T> inline T* MakePointer( PVOID thisptr, int offset ) { return reinterpret_cast<T*>( reinterpret_cast<UINT_PTR>( thisptr ) + offset ); }\n";
